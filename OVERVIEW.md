...
- **翻译**：`DeepL` 免费版（`api-free.deepl.com/v2/translate`），支持 EN/ZH/RU，Key 在设置中填写
- **同传朗读**：`flutter_tts`（英文 en-US / 俄语 ru-RU）
- **设置持久化**：`shared_preferences`（引擎、外语、DeepL Key、语速、字幕字号）
...
- `lib/services/translation_service.dart`：DeepL 翻译
- `lib/pages/live_page.dart`：主界面（大字幕 + 开始/停止）
- `lib/pages/settings_page.dart`：设置（外语切换、引擎、DeepL Key、语速、字号）
...
- DeepL 免费额度约 50 万字符/月，个人足够；Key 在 deepl.com/pro-api 申请 Free 套餐
